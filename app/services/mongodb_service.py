...
class MongoDBService:
            ...
            if existe_registro:
                if coleccion == 'RepresentacionCodificacion':
                    self.collection.delete_one(
                        {"nombre_algoritmo": datos_json['nombre_algoritmo'], "nombre_dataset": datos_json['nombre_dataset']})
                elif coleccion == 'Dataset':
                    self.collection.delete_one(
                        {"nombreDoc": datos_json['nombreDoc'], "version": datos_json['version']})
    ...
    def guardar_json_metricas(self, datos_json, coleccion):
        try:
            ...
            print(
                f'Error al guardar o actualizar el JSON en MongoDB: {str(e)}')
            return None
    ...
    def obtener_datos_algoritmo(self, coleccion, nombre_algoritmo, nombre_dataset):
        try:
            ...
            query = {"nombre_dataset": {"$regex": regex_pattern,
                                        "$options": "i"}, "nombre_algoritmo": nombre_algoritmo.upper()}
    ...
    def obtener_registros_metricas_recientes(self, coleccion, nombre_dataset):
        try:
            fecha_mas_reciente = self.obtener_fecha_mas_reciente(
                coleccion, nombre_dataset)
            ...
            print(
                f'Error al obtener los registros de la fecha más reciente: {str(e)}')
            return None
    '''
    ...
    def obtener_mejores_algoritmos(self, coleccion, nombre_dataset):
        try:
            registros = self.obtener_registros_metricas_recientes(
                coleccion, nombre_dataset)
