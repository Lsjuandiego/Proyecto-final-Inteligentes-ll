...
class MLearningService:
    ...
    def validaciones(self, entrenamiento: InfoEntrenamiento, nombre_dataset):
        ...
        validar_columnas = self.validar_columnas(
            entrenamiento.columnas_x, nombre_dataset)
        if validar_columnas:
            validacion2 = self.validar_columnas(
                entrenamiento.objetivo_y, nombre_dataset)
            if validacion2:
    ...
    def knn(self, entrenamiento: InfoEntrenamiento, nombre_dataset):
        ...
        if validaciones is True:
            dataframe = self.preparacion_dataframe(
                entrenamiento, nombre_dataset)
            if dataframe is not None:
                self.determinar_x_y(
                    dataframe, entrenamiento.columnas_x, entrenamiento.objetivo_y)
                if entrenamiento.tecnica == "hold-out":
                    ...
                    self.guardar_info_modelos(
                        'KNN', entrenamiento.normalizacion, entrenamiento.tecnica, metricas, matriz.tolist())
                    return metricas
                elif entrenamiento.tecnica == "cross-validation":
                    # print("cantidad", entrenamiento.cantidad)
                    self.modeloKNN = KNeighborsClassifier(
                        n_neighbors=self.find_best_n_neighbors())
                    metricas, matriz = self.validacion_cruzada(
                        self.modeloKNN, entrenamiento.cantidad, 'knn')
                    self.guardar_info_modelos(
                        'KNN', entrenamiento.normalizacion, entrenamiento.tecnica, metricas, matriz.tolist())
                    return metricas
            else:
        ...
        if validaciones is True:
            ...
            if dataframe is not None:
                ...
                if entrenamiento.tecnica == "hold-out":
                    ...
                    metricas = self.metricas_hold_out()
                    self.guardar_info_modelos(
                        'Regresión Logística', entrenamiento.normalizacion, entrenamiento.tecnica, metricas, matriz.tolist())
                    return metricas
                elif entrenamiento.tecnica == "cross-validation":
                    self.modeloRegLog = LogisticRegression()
                    metricas, matriz = self.validacion_cruzada(
                        self.modeloRegLog, entrenamiento.cantidad, 'reglog')
                    self.guardar_info_modelos(
                        'Regresión Logística', entrenamiento.normalizacion, entrenamiento.tecnica, metricas, matriz.tolist())
                    return metricas
        return "ok"
        ...
        if validaciones is True:
            ...
            if dataframe is not None:
                ...
                if entrenamiento.tecnica == "hold-out":
                    ...
                    metricas = self.metricas_hold_out()
                    self.guardar_info_modelos(
                        'Naive Bayes', entrenamiento.normalizacion, entrenamiento.tecnica, metricas, matriz.tolist())
                    return metricas
                elif entrenamiento.tecnica == "cross-validation":
                    modelo = GaussianNB()
                    metricas, matriz = self.validacion_cruzada(
                        modelo, entrenamiento.cantidad, 'naive_bayes')
                    self.guardar_info_modelos(
                        'Naive Bayes', entrenamiento.normalizacion, entrenamiento.tecnica, metricas, matriz.tolist())
                    return metricas
            else:
        ...
        if validaciones is True:
            ...
            if dataframe is not None:
                ...
                if entrenamiento.tecnica == "hold-out":
                    ...
                    metricas = self.metricas_hold_out()
                    self.guardar_info_modelos(
                        'Árbol de decisión', entrenamiento.normalizacion, entrenamiento.tecnica, metricas, matriz.tolist())
                    return metricas
                elif entrenamiento.tecnica == "cross-validation":
                    metricas, matriz = self.validacion_cruzada(
                        modelo, entrenamiento.cantidad, 'arbol_decision')
                    self.guardar_info_modelos(
                        'Árbol de decisión', entrenamiento.normalizacion, entrenamiento.tecnica, metricas, matriz.tolist())
                    return metricas
            else:
        ...
        if validaciones is True:
            ...
            if dataframe is not None:
                ...
                if entrenamiento.tecnica == "hold-out":
                    ...
                    self.guardar_info_modelos(
                        'Regresión Líneal', entrenamiento.normalizacion, entrenamiento.tecnica, metricas, None)
                    return metricas
                elif entrenamiento.tecnica == "cross-validation":
                    modelo = LinearRegression()
                    metricas = self.validacion_cruzada_regresionLineal(
                        modelo, entrenamiento.cantidad)
                    self.guardar_info_modelos(
                        'Regresión Líneal', entrenamiento.normalizacion, entrenamiento.tecnica, metricas, None)
                    return metricas
            else:
    ...
    def validacion_cruzada_regresionLineal(self, modelo, cv):
        scores = cross_val_score(
            modelo, self.x, self.y, cv=cv, scoring='neg_mean_squared_error')
        mse_scores = -scores  # convertir a positivo
        rmse_scores = np.sqrt(mse_scores)
        r2_scores = cross_val_score(
            modelo, self.x, self.y, cv=cv, scoring='r2')
    ...
    def preparacion_dataframe(self, entrenamiento: InfoEntrenamiento, nombre_dataset):
        try:
            ...
            if datos:
                ...
                df = pd.DataFrame(valores_seleccionados,
                                  columns=list(titulos_seleccionados))
                # print("DATAFRAME ", df)
                # print(df)
                ...
                print("-------------------------------------------")
                # print("CODIFICAR")
                df = self.dataframe_service.codificar_valores_cat(
                    df, entrenamiento, self.n_dataset)
                print("NORMALIZAR")
                dataNumerica = self.dataframe_service.normalizar_informacion(
                    dataframe=df, tipo=entrenamiento.normalizacion, objetivo_y=entrenamiento.objetivo_y)
                # print(df)
                print("REDONDEAR")
                dataNumerica = self.dataframe_service.redondear_datos(
                    dataNumerica)
                # print(df)
            ...
            if dataframe is not None:
                ...
                if entrenamiento.tecnica == "hold-out":
                    ...
                    if matriz is None:
                        return "Error al obtener la matriz de confusión"
                    metricas = self.metricas_hold_out()
                    self.guardar_info_modelos('Máquinas de soporte vectorial (SVM)',
                                              entrenamiento.normalizacion, entrenamiento.tecnica, metricas, matriz.tolist())
                    return metricas
                elif entrenamiento.tecnica == "cross-validation":
                    modelo = SVC(C=1.0, kernel='rbf', gamma='scale')
                    metricas, matriz = self.validacion_cruzada(
                        modelo, entrenamiento.cantidad, 'svm')
                    self.guardar_info_modelos('Máquinas de soporte vectorial (SVM)',
                                              entrenamiento.normalizacion, entrenamiento.tecnica, metricas, matriz.tolist())
                    return metricas
            else:
    ...
    def guardar_info_modelos(self, nombre_modelo, normalizacion, tecnica, metricas, matriz):
        ...
        if matriz is not None:
            info = {'fecha': fecha_actual, "nombre_dataset": self.n_dataset, 'nombre_algoritmo': nombre_modelo,
                    'normalizacion': normalizacion, 'tecnica': tecnica, 'metricas': metricas, 'matriz_confusion': matriz}
        else:
            info = {'fecha': fecha_actual, "nombre_dataset": self.n_dataset, 'nombre_algoritmo': nombre_modelo,
                    'normalizacion': normalizacion, 'tecnica': tecnica, 'metricas': metricas}
        id = self.mongo_service.guardar_json_metricas(
            info, 'InformacionModelos')
        print("ID ", id)
    ...
    def particion_dataset(self, dataframe, porcentaje):
        ...
        self.XTrain, self.XTest, self.yTrain, self.yTest = train_test_split(
            self.x, self.y, test_size=porcentaje, random_state=2)
    ...
    def determinar_x_y(self, dataframe, columnas, objetivo):
        if not isinstance(dataframe, pd.DataFrame):
            raise TypeError(
                "El parámetro 'dataframe' debe ser un DataFrame de pandas.")
        else:
    ...
    def obtener_matriz_confusion(self, nombre_modelo):
        try:
            ...
            plt.savefig(os.path.join(ruta_guardado, f"{
                        self.n_dataset}-{nombre_modelo}-ho-matriz_confusion.png"))
            plt.close()
            return matrizKNN
        except Exception as e:
            return None
    ...
    def metricas_hold_out(self):
        # proporción de predicciones correctas del modelo
        accuracy = accuracy_score(self.yTest, self.yPredict)
        # proporción de predicciones positivas que fueron correctas
        precision = precision_score(
            self.yTest, self.yPredict, average='weighted', zero_division=1)
        # proporción de positivos reales que se identificaron correctamente
        recall = recall_score(self.yTest, self.yPredict, average='weighted')
        # medida armónica de precision y recall
        f1 = f1_score(self.yTest, self.yPredict, average='weighted')
    ...
    def validar_columnas(self, columnas, nombre_dataset):
        self.disponibles = self.processing_service.columnas_disponibles_dataset(
            nombre_dataset)
        self.disponibles = self.reemplazar_caracteres_especiales(
            self.disponibles)
    ...
    def validacion_cruzada(self, modelo, cv, nombre_modelo):
        ...
        resultados = cross_validate(
            modelo, self.x, self.y, cv=cv, scoring=scoring)
       ...
       # Obtener las métricas promedio
        ...
        os.makedirs(ruta_guardado, exist_ok=True)
        plt.savefig(os.path.join(ruta_guardado, f"{
                    self.n_dataset}-{nombre_modelo}-cv-matriz_confusion.png"))
        plt.close()
    ...
    def guardar_modelo(self, modelo, nombre_modelo):
        ...
        try:
            ...
            ruta_modelo = os.path.join(
                ruta_directorio, f'{self.n_dataset}-{nombre_modelo}.pkl')
    ...
    def prediccion(self, prediccion: PrediccionModel, nombre_dataset):
        try:
            ...
            datos = self.mongo_service.obtener_datos_algoritmo(
                'RepresentacionCodificacion', prediccion.algoritmo, nombre_dataset)
            ...
            if datos:
                ...
                columns_predecir = [valor.lower()
                                    for valor in prediccion.valores_predecir.keys()]
                clavesReemplazadas = []
                ...
                if cant == len(columns_x) and len(columns_x) == len(columns_predecir):
                    ...
                    prediccion = self.__codificar_valores_recibidos(
                        datos, columns_x, prediccion, clavesReemplazadas)
                    # Cargar el modelo desde el archivo
                    ruta_modelo = 'app/files/modelos/'
                    modelo = self.__obtener_modelo(
                        ruta_modelo, prediccion.algoritmo, nombre_dataset)
                    # Crear un dataframe con los datos codificados
                    ejemplo_prueba = pd.DataFrame(
                        prediccion.valores_predecir, index=[0])
                    if modelo:
    ...
    def obtener_top3_algoritmos(self, nombre_dataset):
        if self.file_service.verificar_dataset(nombre_dataset) is False:
            return f"No existe el dataset {nombre_dataset}"
        metricas = self.mongo_service.obtener_mejores_algoritmos(
            'InformacionModelos', nombre_dataset=nombre_dataset)
        datos = []
        if metricas:
            if len(metricas) > 0:
                # Ordenar la lista en función de la precisión en orden descendente
                dic_ordenado = sorted(metricas, key=lambda x: x['metricas'].get(
                    'accuracy', -1), reverse=True)

                # Obtener los tres primeros elementos
                mejores_tres = dic_ordenado[:3]
                for data in mejores_tres:
                    datos.append({'nombre_algoritmo': data['nombre_algoritmo'], 'normalizacion': data[
                                 'normalizacion'], 'tecnica': data['tecnica'], 'metricas': data['metricas']})
                    # print(datos)
