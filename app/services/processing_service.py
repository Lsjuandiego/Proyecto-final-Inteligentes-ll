...
class ProcessingService:
            ...
            if datos:
                ...
                if len(columnas_con_nulos) > 0:
                    ...
                    if df.isnull().values.any():
                        print(
                            "Aún hay valores nulos en el DataFrame después del descarte.")
                    else:
        ...
        try:
            ...
            if datos:
                ...
                if df.isnull().values.any():
                    ...
                    self.file_service.guardar_archivo_json(
                        "imputacion", nombre_archivo, 'cleanData', 'Dataset')
                    # Guarda una copia del DataFrame modificado en un archivo Excel
    ...
    def generar_img_analisis(self, nombre_dataset):
        try:
            ...
            if datos:
                ...
                ubicacion_matriz = self.file_service.obtener_ultimo_archivo(
                    f"imgs//matriz_correlacion/{nombre_dataset}-matriz_correlacion")
    ...
    def _generate_histograma(self, columnas_numericas, nombre_dataset):
        ...
        self._generate_matriz_correlacion(
            columnas_numericas, plt, nombre_dataset)
    ...
    def _generate_matriz_correlacion(self, columnas_numericas, plt, nombre_dataset):
        ...
        sb.heatmap(columnas_numericas.astype(float).corr(), linewidths=0.1,
    ...
    def obtener_ultima_matriz_confusion_algoritmo(self, nombre: str, nombre_dataset):
        try:
            ...
            archivos = glob.glob(
                f"app/files/imgs/modelos/matrices-confusion/{nombre_dataset}*{nombre}*.png")
            aux = []
    ...
    def obtener_metricas_algoritmos(self, nombre_dataset):
        if self.file_service.verificar_dataset(nombre_dataset) is False:
            return f"No existe el dataset {nombre_dataset}"
        metricas = self.mongo_service.obtener_registros_metricas_recientes(
            'InformacionModelos', nombre_dataset)
        datos = []
        # print(metricas)
