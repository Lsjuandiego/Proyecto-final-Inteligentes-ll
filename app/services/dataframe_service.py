...
# # Obtener los valor originales y codificado de cada columna elemento de las columnas
            ...
            for valor_codificado, valor_original in zip(dataframe[col], valores_originales[col]):
                if col == entrenamiento.objetivo_y:
                    if valor_codificado not in aux:
                        listY.append(
                            {"valor_codificado": valor_codificado, "valor_original": valor_original})
        ...
        if len(listY) > 0:
            #  print("guardar json")
            id = self.mongo_service.guardar_json({"nombre_dataset": nombre_dataset, "nombre_algoritmo": entrenamiento.nombre_algoritmo,
                                                 "datosY": listY, "datosX": listX, 'x': entrenamiento.columnas_x, 'y': entrenamiento.objetivo_y}, "RepresentacionCodificacion")
        return dataframe
    ...
    def normalizar_informacion(self, dataframe, tipo, objetivo_y):
        ...
        if tipo == "min-max":
            ...
            dataNumerica = pd.DataFrame(escalador.fit_transform(
                dataNumerica), columns=dataNumerica.columns)
            print("min-max")
            # print(dataNumerica)
            return dataNumerica
