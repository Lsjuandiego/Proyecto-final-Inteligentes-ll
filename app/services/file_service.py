...
class FileService:
    ...
    def obtener_ultimo_archivo(self, ubicacion):
        try:
            ...
            if 'imgs' in ubicacion:
                archivo = glob.glob(ruta_carpeta + '.png') + \
                    glob.glob(ruta_carpeta + '.jpg')
            else:
                archivo = glob.glob(os.path.join(
                    ruta_carpeta, '*.xlsx')) + glob.glob(os.path.join(ruta_carpeta, '*.csv'))
            if archivo:
    ...
    def convertir_archivo_a_json(self, version, filename, ubicacion):
        try:
            ...
            if ultimo_archivo:
                ...
                datos_json = {'nombreDoc': filename, 'nombre_dataset': nombre_dataset,
                              'version': version, 'titulos': titulos, 'valores': valores}
                return datos_json
            else:
    ...
    def _copia_excel(self, df, nombre_archivo, accion):
        try:
            ruta_archivo = './app/files/cleanData/' + \
                accion+os.path.basename(nombre_archivo)
